    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!cachedToolpath) {
            try {
                ...
                yield exec.exec(`ls ${path.dirname(kubectlDownloadPath)}`);
                if (os.type() !== 'Windows_NT') {
                    yield io.cp(kubectlDownloadPath, path.join(path.dirname(kubectlDownloadPath), 'tool.tar.gz'));
                    kubectlDownloadPath = path.join(path.dirname(kubectlDownloadPath), 'tool.tar.gz');
                }
                switch (os.type()) {
                    case 'Linux':
                        kubectlDownloadPath = yield toolCache.extractTar(kubectlDownloadPath);
                        break;
                    case 'Darwin':
                        kubectlDownloadPath = yield toolCache.extractTar(kubectlDownloadPath);
                        break;
                    case 'Windows_NT':
                    default:
                        kubectlDownloadPath = yield toolCache.extractZip(kubectlDownloadPath);
                }
            }
            catch (exception) {
